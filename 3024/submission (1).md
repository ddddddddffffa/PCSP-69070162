...
```text
รับคะแนน 0-10 เป็นจำนวณจริงเมื่อโหวตเสร็จระบบจะแสดง ผลรวมคะแนน ถ้าคะแนนสูงสุด กับ คะแนนต่ำสุดห่างกันมากกว่า 2 output จะเป็น surprising แต่ถ้า < 2 จะเป็น Not surprising
input คะแนนรวม , คะแนนศุงสุดของการโหวต
output surprising ถ้า x > 2 , notsurprising ถ้า x < 2 (x = สูงสุด - ต่ำสุด)
constraints ต้องจำลองค่าต่ำสุดเอง
```
...
```text
step1 : รับ input
step 2 : หาค่า ตรงกลาง+ต่ำสุด โดย min+mid= total-max
step 3 : หาค่าน้อยสุดแบบจำลองโดยให้ min = min+mid - max
step 4 :ใช้ if เพื่อสร้างเงื่อนไขถ้า max - min > 2 ให้ print surprising
step 5 : ถ้าไม่ให้ print not surprising
```
...
```text
วิธีสุดท้ายเหมือนกับแผนแรก รับ input หาค่า ตรงกลาง+ต่ำสุด โดย min+mid= total-max
หาค่าน้อยสุดแบบจำลองโดยให้ min = min+mid - max ใช้ if เพื่อสร้างเงื่อนไขถ้า max - min > 2 ให้ print surprising  ถ้าไม่ให้ print not surprising
```
...
```text
เพื่อทดสอบว่า code ใช้ได้จริงไหม
```
