...
```text
ปลูกดอกไม้ในตารางตามลำดับช่อง โดยแบ่งการปลูกออกเป็น แนวทแยง ที่มีความหนาแถบละ ช่อง โจทย์ต้องการหาว่า ช่องที่ ช่องสุดท้ายตกอยู่ในแถบที่เท่าไหร่
input : ความหนา และ จำนวนช่องที่ปลูก
output : เลขช่องของ n
constrains : ความหนาของแถบ เป็นจำนวนเต็มตั้งแต่ 1 ถึง 10 และ จำนวนช่องที่ปลูกทั้งหมด เป็นจำนวนเต็มตั้งแต่ 1 ถึง 100,000
```
...
1: อ่านค่า L และ N
2: กำหนด band = 1
3: คำนวณจำนวนช่องสะสม c = (band * L) * (band * L + 1) // 2
4: ถ้า N <= c ให้พิมพ์ค่า band แล้วจบการทำงาน
5: ถ้ายังไม่จริง ให้เพิ่ม band ขึ้น 1 แล้วกลับไปทำ Step 3
```
...
```text
เพื่อดูว่าโปรแกรมทำงานถูกต้องตั้งแต่ช่องแรก
```
